...
import SimpleGridDisplay, {
  DisplayName
} from 'src/core/shared/display/SimpleGridDisplay';
import { useAppDispatch, useAppSelector } from 'src/store/appHook';
import LoadingLogo from 'src/shared/loading/full-logo/LoadingLogo';
import { useFetchPlanetsQuery } from 'src/core/store/swapi/swapi';
import {
  selectPage,
  selectTotalPages
} from 'src/core/store/swapi/swapi.selectors';
...
function PlanetsAll() {
  ...
  const { data, isFetching, isLoading, error, isSuccess, isError, refetch } =
  ...
  const onPageHandler = (_: React.ChangeEvent<unknown>, page: number) => {
    dispatch(
      dispatchPaging({ entityId: ENTITY_NAME, pagination: { page: page } })
    );
  };

  const onResultSelectHandler = (
  ...
  return (
    <Stack direction="column" width="100%">
      ...
      <Box mt={isMobile ? 8 : 10} mx={isMobile ? 2 : 0}>
        <SimpleGridDisplay
          data={data.results}
          itemUrlPath={ENTITY_NAME}
          totalPages={totalPages}
          page={page}
          onPaged={onPageHandler}
          totalRecords={data.total_records}
          isFetching={isFetching}
          data-testId="planet-grid"
        />
      </Box>
    </Stack>
  );
