...
export interface SearchAutoCompleteProps {
  entity: string;
  onResultSelect: (
    selection: ResultProperty<StarwarsSearchable> | null
  ) => void;
}
...
function SearchAutoComplete({
  ...
  const [options, setOptions] = useState<
    readonly ResultProperty<StarwarsSearchable>[]
  >([]);
  // dropdown selection
  const [value, setValue] = useState<ResultProperty<StarwarsSearchable> | null>(
    null
  );
  // user text input
  ...
  const { data, isFetching, error, isError, refetch } = useSearchContentQuery(
    debouncedInputValue
      ? {
          entity,
  ...
  useUpdateEffect(() => {}, [debouncedInputValue]);
  ...
  const onChangeHandler = (
    selected: ResultProperty<StarwarsSearchable> | null
  ) => {
    ...
    >
      <Autocomplete
        ...
        renderInput={(params: AutocompleteRenderInputParams) => (
          <AutoCompleteTextField params={params} entity={entity} />
        )}
        onChange={(
          event: any,
          newValue: ResultProperty<StarwarsSearchable> | null
        ) => {
        ...
        isOptionEqualToValue={(
          option: ResultProperty<StarwarsSearchable>,
          value: ResultProperty<StarwarsSearchable>
        ) => {
          return option.uid === value.uid;
        }}
      />
      <Box ml={2}>{isFetching && <ProgressCircle size={20} />}</Box>
    </Stack>
  );
}
...
function AutoCompleteTextField({
  ...
  return (
    <TextField {...params} label={`Search for ${entity}`} variant="standard" />
  );
}
