...
function SimpleGridDisplay({
  ...
  return (
    <LayoutWithGutter size={gutterSize}>
      {totalPages > 0 && (
        <Grid xs={12} mb={2} id="top-pagination">
          <Stack
            direction="row"
          ...
          >
            ...
            <Box>
              <Typography color="text.secondary" sx={{ lineHeight: 1 }}>
                {(page - 1) * PAGE_LIMIT_30 + 1} -{' '}
                {page === totalPages ? totalRecords : page * PAGE_LIMIT_30} of{' '}
                {totalRecords}
              </Typography>
            </Box>
          </Stack>
