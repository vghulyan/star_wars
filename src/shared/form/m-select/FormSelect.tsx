...
import {
  FormSelectProps,
  NONE_SELECTED_VALUE
} from 'src/shared/models/form.model';
    ...
    <FormControl fullWidth size="small">
        ...
        {props.useDefaultNoneSelected && (
          <MenuItem value={NONE_SELECTED_VALUE} key={''} disabled>
            <Typography sx={{ color: '#666' }}>
              Select {props.useDefaultNoneSelected}
            </Typography>
          </MenuItem>
        )}
        {options?.map((opt) => {
          ...
          return (
            <MenuItem value={value} key={opt.id ?? opt.uid}>
              {display}
            </MenuItem>
          );
        })}
      ...
      >
        {formik.touched[name] && formik.errors[name] ? (
          <ErrorMessage name={name} render={(err) => upperFirst(err)} />
        ) : (
          <>{props.helperText}</>
        )}
      </FormHelperText>
